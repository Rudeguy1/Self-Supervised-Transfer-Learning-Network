...
class setup_config():
    ...
    def __init__(self, 
                 ...
                 len_borderxstart=None,
                 len_borderxsend=None,
                 len_borderystart=None,
                 len_borderysend=None,
        ...
        self.len_borderxstart = len_borderxstart
        self.len_borderxsend = len_borderxsend
        self.len_borderystart = len_borderystart
        self.len_borderysend = len_borderysend
        self.scale = scale
...
config = setup_config(input_rows=options.input_rows,
                      ...
                      scale=options.scale,
                      len_borderxstart= 145,
                      len_borderxsend = 166,
                      len_borderystart = 200,
                      len_borderysend = 206,
                      len_depth=3,
                      lung_min=0.7,
                      lung_max = 0.25,
...
def infinite_generator_from_one_volume(config, img_array):
    ...
    len_borderzstart = math.floor(size_z * 0.28)
    len_borderzsend = size_z - math.ceil(size_z * 0.8)
    if size_z - config.input_deps - config.len_depth -  len_borderzsend  < len_borderzstart :
        return None
    ...
    while True:
        ...
        start_x = random.randint(0 + config.len_borderxstart, size_x  - config.len_borderxsend)
        start_y = random.randint(0 + config.len_borderystart, size_y   - config.len_borderysend)

        start_z = random.randint(0 + len_borderzstart, size_z - len_borderzsend - config.input_deps)
        ...
        d_test = np.zeros((config.input_rows, config.input_cols, config.input_deps), dtype=float)
        #-----------------------the HU_thred may like differ organ simplely selact
            ...
            for i in range(config.input_rows):
                for j in range(config.input_cols):
                    for k in range(config.len_depth):
                        if crop_window[i, j, d+k] >= 0.53 and crop_window[i, j, d+k] < 0.555:
...
def get_self_learning_data(fold, config):
    ...
    for index_subset in fold:
        ...
        file_list = glob(os.path.join(luna_subset_path, "*.gz"))
